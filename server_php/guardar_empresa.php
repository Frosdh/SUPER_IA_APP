...
// Datos enviados desde levantamiento_empresa.php
...
try {
    ...
    $st->execute([
        ...
        $asesor_id ?: null,
        trim($_POST['ciudad_empresa'] ?? ''),
        trim($_POST['zona_empresa'] ?? ''),
        floatOrNull($_POST['lat'] ?? ''),
        floatOrNull($_POST['lng'] ?? ''),
        $cliente_id
    ]);

    // ========== PASO 2: Obtener o crear ENCUESTA_COMERCIAL (para Paso 4 - interés productos) ==========
    // Primero buscamos si ya existe una encuesta_comercial sin terminar
    $st = $pdo->prepare(
        "SELECT id FROM encuesta_comercial WHERE tarea_id = (
            SELECT id FROM tarea WHERE cliente_prospecto_id = ? AND tipo_tarea = 'levantamiento_empresa' 
            ORDER BY created_at DESC LIMIT 1
        ) LIMIT 1"
    );
    $st->execute([$cliente_id]);
    $encuesta_comercial = $st->fetch(PDO::FETCH_ASSOC);
    
    if ($encuesta_comercial) {
        $encuesta_comercial_id = $encuesta_comercial['id'];
    } else {
        $encuesta_comercial_id = genUUID();
        // Crearemos la encuesta_comercial más adelante con datos de Paso 4
    }
    ...
    // Si no existe, creamos una dummy (se vincula con tarea)
    if (!$encuesta_cred) {
    ...
    // Procesar productos del array prod[i]
    $i = 0;
    while (isset($_POST["prod[$i][nombre]"]) && trim($_POST["prod[$i][nombre]"]) !== '') {
        ...
        $st->execute([
            $prod_id,
            $encuesta_cred_id,
            trim($_POST["prod[$i][nombre]"] ?? ''),
            floatOrNull($_POST["prod[$i][precio_venta]"] ?? ''),
            floatOrNull($_POST["prod[$i][costo]"] ?? ''),
            intOrNull($_POST["prod[$i][cantidad]"] ?? ''),
            floatOrNull($_POST["prod[$i][margen]"] ?? ''),
            floatOrNull($_POST["prod[$i][total_venta_mes]"] ?? ''),
            intOrNull($_POST["prod[$i][inventario]"] ?? ''),
            floatOrNull($_POST["prod[$i][compra_sem]"] ?? '')
        ]);
        $i++;
    }
    ...
    $st->execute([
        ...
        $tarea_id,
        boolToTinyInt($_POST['mantiene_ahorro'] ?? ''),
        boolToTinyInt($_POST['mantiene_corriente'] ?? ''),
        boolToTinyInt($_POST['tiene_inversiones'] ?? ''),
        trim($_POST['institucion_inversiones'] ?? ''),
        floatOrNull($_POST['valor_inversion'] ?? ''),
        trim($_POST['plazo_inversion'] ?? ''),
        trim($_POST['fecha_vencimiento_inversion'] ?? '') ?: null,
        boolToTinyInt($_POST['interes_propuesta_previa'] ?? ''),
        trim($_POST['fecha_nuevo_contacto'] ?? '') ?: null,
        boolToTinyInt($_POST['tiene_operaciones_crediticias'] ?? ''),
        trim($_POST['institucion_credito'] ?? ''),
        boolToTinyInt($_POST['interes_conocer_productos'] ?? ''),
        boolToTinyInt($_POST['solicitar_corriente'] ?? ''),
        boolToTinyInt($_POST['solicitar_ahorro'] ?? ''),
        boolToTinyInt($_POST['solicitar_inversion'] ?? ''),
        boolToTinyInt($_POST['solicitar_credito'] ?? ''),
        boolToTinyInt($_POST['razon_ya_trabaja'] ?? ''),
        boolToTinyInt($_POST['razon_desconfia'] ?? ''),
        boolToTinyInt($_POST['razon_agusto'] ?? ''),
        boolToTinyInt($_POST['razon_mala_exp'] ?? ''),
