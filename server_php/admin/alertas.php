...
if ($user_role === 'super_admin' || $user_role === 'admin') {
    ...
    $query = "
        SELECT a.id as id_alerta, c.nombre, c.apellidos, a.descripcion as mensaje, a.tipo_alerta as tipo, 
               a.estado, a.fecha_creacion as fecha, u.nombres as asesor_nombre
        FROM alertas a
    ...
    // Supervisor ve alertas de sus asesores
    $query = "
        SELECT a.id as id_alerta, c.nombre, c.apellidos, a.descripcion as mensaje, a.tipo_alerta as tipo, 
               a.estado, a.fecha_creacion as fecha, u.nombres as asesor_nombre
        FROM alertas a
    ...
    // Asesor ve solo sus alertas
    $query = "
        SELECT a.id as id_alerta, c.nombre, c.apellidos, a.descripcion as mensaje, a.tipo_alerta as tipo, 
               a.estado, a.fecha_creacion as fecha
        FROM alertas a
        LEFT JOIN clientes c ON a.cliente_id = c.id_cliente
...
$alertas = $pdo->query($query)->fetchAll();
...
if ($user_role === 'super_admin' || $user_role === 'admin') {
    $stats = $pdo->query("
        SELECT 
            ...
            SUM(estado='abierta') as pendientes,
            SUM(estado='cerrada') as revisadas
        FROM alertas
    ")->fetch();
} elseif ($user_role === 'supervisor') {
    $stats = $pdo->query("
        SELECT 
            COUNT(*) as total_alertas,
            SUM(a.estado='abierta') as pendientes,
            SUM(a.estado='cerrada') as revisadas
        FROM alertas a
        LEFT JOIN clientes c ON a.cliente_id = c.id_cliente
        WHERE c.asesor_id_fk IN (
            SELECT id_usuario FROM usuarios WHERE supervisor_id_fk = $user_id
...
<?php else: ?>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .badge-pendiente { background: #ef4444; color: white; }
        .badge-revisada { background: #10b981; color: white; }
        ::-webkit-scrollbar { width: 6px; }
        ::-webkit-scrollbar-track { background: transparent; }
        ::-webkit-scrollbar-thumb { background: #d1d5db; border-radius: 3px; }
<?php endif; ?>
            ...
            <table class="table table-hover">
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        ...
                        <tr>
                            <td><strong>#<?php echo $alerta['id_alerta']; ?></strong></td>
                            <td>
                                <span class="badge" style="background: #3182fe; color: white;">
                                    <?php echo htmlspecialchars($alerta['tipo']); ?>
                                </span>
                            </td>
                            <td><?php echo htmlspecialchars(substr($alerta['mensaje'], 0, 50) . (strlen($alerta['mensaje']) > 50 ? '...' : '')); ?></td>
                            <td><?php echo htmlspecialchars(($alerta['nombre'] ?? 'Sin cliente') . ' ' . ($alerta['apellidos'] ?? '')); ?></td>
                            <?php if ($col_asesor): ?>
                            <td><?php echo htmlspecialchars($alerta['asesor_nombre'] ?? 'Sin asignar'); ?></td>
                            <?php endif; ?>
                            <td><?php echo date('d/m/Y H:i', strtotime($alerta['fecha'])); ?></td>
                            <td>
                                <?php 
                                $estado = strtolower($alerta['estado']);
                                if ($estado === 'abierta'): ?>
                                    <span class="badge badge-pendiente">⏳ Abierta</span>
                                <?php elseif ($estado === 'en_proceso'): ?>
                                    <span class="badge" style="background: #f59e0b; color: white;">🔄 En Proceso</span>
                                <?php else: ?>
                                    <span class="badge badge-revisada">✓ Cerrada</span>
                                <?php endif; ?>
                            </td>
                            <td>
