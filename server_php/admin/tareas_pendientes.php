...
if ($tipo) { $where[] = 't.tipo_tarea = :tipo'; $params[':tipo'] = $tipo; }
if ($q !== '') {
...
$tareas_pendientes = 0;
foreach ($tareas as $t) if (($t['estado'] ?? '') !== 'completada') $tareas_pendientes++;
...
<head>
    ...
    <style>
        ...
        *{margin:0;padding:0;box-sizing:border-box;}
        ...
        .page-header h1{font-size:24px;font-weight:800;margin:0;display:flex;align-items:center;gap:10px;}
        .page-header h1 i{color:var(--brand-yellow-deep);}

        .filter-bar{background:#fff;border:1px solid var(--brand-border);border-radius:14px;padding:14px 16px;display:flex;flex-wrap:wrap;gap:10px;align-items:center;margin-bottom:18px;box-shadow:var(--brand-shadow-sm);}
        .chips{display:flex;gap:6px;flex-wrap:wrap;}
        .chip{padding:7px 14px;border-radius:9px;background:#f3f4f6;color:#374151;text-decoration:none;font-weight:700;font-size:13px;border:1px solid transparent;}
        .chip:hover{background:#e5e7eb;color:#111827;}
        .chip.active{background:var(--brand-navy-deep);color:#fff;}
        .filter-bar form{display:flex;gap:10px;align-items:center;margin-left:auto;flex-wrap:wrap;}
        .filter-bar select,.filter-bar input{padding:8px 12px;border:1.5px solid var(--brand-border);border-radius:9px;font-size:13.5px;background:#fff;}
        .filter-bar input{min-width:220px;}
        .filter-bar .btn-search{background:var(--brand-yellow);color:var(--brand-navy-deep);font-weight:800;padding:8px 16px;border:none;border-radius:9px;cursor:pointer;}

        .grid-tareas{display:grid;grid-template-columns:repeat(auto-fill,minmax(310px,1fr));gap:14px;}
        .card-tarea{background:#fff;border:1px solid var(--brand-border);border-radius:14px;padding:16px;box-shadow:var(--brand-shadow-sm);transition:.2s;display:flex;flex-direction:column;gap:10px;border-left:4px solid var(--brand-navy);}
        .card-tarea:hover{transform:translateY(-2px);box-shadow:0 12px 24px rgba(18,58,109,.10);}
        .card-tarea.done{border-left-color:#10b981;background:#fafefb;}
        .card-tarea.late{border-left-color:#dc2626;}
        .ct-head{display:flex;align-items:center;gap:10px;}
        .ct-ico{width:40px;height:40px;border-radius:11px;display:flex;align-items:center;justify-content:center;color:#fff;font-size:16px;flex-shrink:0;}
        .ct-info{flex:1;min-width:0;}
        .ct-name{font-weight:800;font-size:15px;line-height:1.25;color:var(--brand-navy-deep);}
        .ct-tipo{font-size:11.5px;text-transform:uppercase;font-weight:700;color:var(--brand-gray);letter-spacing:.3px;margin-top:2px;}
        .ct-meta{font-size:12.5px;color:#374151;display:grid;grid-template-columns:1fr 1fr;gap:4px 10px;}
        .ct-meta div{display:flex;align-items:center;gap:6px;}
        .ct-meta i{color:var(--brand-navy);font-size:11px;}
        .ct-actions{display:flex;gap:8px;margin-top:4px;}
        .btn-act{flex:1;padding:8px 10px;border-radius:9px;font-weight:700;font-size:12.5px;text-decoration:none;display:inline-flex;align-items:center;justify-content:center;gap:6px;border:none;cursor:pointer;}
        .btn-act.primary{background:var(--brand-yellow);color:var(--brand-navy-deep);}
        .btn-act.primary:hover{background:var(--brand-yellow-deep);}
        .btn-act.ghost{background:#f3f4f6;color:#374151;}
        .btn-act.ghost:hover{background:#e5e7eb;}
        .badge-estado{padding:3px 9px;border-radius:6px;font-size:10.5px;font-weight:800;text-transform:uppercase;letter-spacing:.3px;display:inline-block;}
        .be-prog{background:#dbeafe;color:#1e40af;}
        .be-proc{background:#fef3c7;color:#92400e;}
        .be-comp{background:#dcfce7;color:#065f46;}
        .be-post{background:#fee2e2;color:#991b1b;}

        .empty{background:#fff;border:1px dashed var(--brand-border);border-radius:14px;padding:50px 20px;text-align:center;color:#9ca3af;}
    ...
    <div class="content-area">
        ...
        <div class="filter-bar">
            <div class="chips">
                ...
                <a class="chip <?= $filtro==='pendientes' ? 'active' : '' ?>" href="?filtro=pendientes">Solo pendientes</a>
            ...
            <form method="get">
                ...
                <input type="text" name="q" placeholder="Buscar cliente, cédula o teléfono…" value="<?= htmlspecialchars($q) ?>">
            ...
            <div class="grid-tareas">
                ...
                ?>
                    <div class="card-tarea <?= $done?'done':($late?'late':'') ?>">
                        <div class="ct-head">
                            <div class="ct-ico" style="background:<?= $tipo_meta[2] ?>;"><i class="fas <?= $tipo_meta[1] ?>"></i></div>
                            <div class="ct-info">
                                <div class="ct-name"><?= htmlspecialchars($t['cliente_nombre'] ?? 'Sin cliente') ?></div>
                                <div class="ct-tipo"><?= htmlspecialchars($tipo_meta[0]) ?>
                                    <?php
                                    $clase = 'be-prog';
                                    if ($estado==='completada') $clase='be-comp';
                                    elseif ($estado==='en_proceso') $clase='be-proc';
                                    elseif ($estado==='postergada') $clase='be-post';
                                    ?>
                                    · <span class="badge-estado <?= $clase ?>"><?= ucfirst(str_replace('_',' ',$estado)) ?></span>
                                </div>
                            </div>
                        </div>
                        <div class="ct-meta">
                            <div><i class="far fa-calendar"></i> <?= $t['fecha_programada'] ? date('d/m/Y', strtotime($t['fecha_programada'])) : '—' ?></div>
                            <div><i class="far fa-clock"></i> <?= $t['hora_programada'] ? date('H:i', strtotime($t['hora_programada'])) : '—' ?></div>
                            <?php if (!empty($t['cedula'])): ?><div><i class="fas fa-id-card"></i> <?= htmlspecialchars($t['cedula']) ?></div><?php endif; ?>
                            <?php if (!empty($t['telefono'])): ?><div><i class="fas fa-phone"></i> <?= htmlspecialchars($t['telefono']) ?></div><?php endif; ?>
                            <?php if (!empty($t['cli_ciudad'] ?? $t['ciudad'])): ?><div><i class="fas fa-city"></i> <?= htmlspecialchars($t['cli_ciudad'] ?? $t['ciudad']) ?></div><?php endif; ?>
                            <?php if (!empty($t['zona'])): ?><div><i class="fas fa-map-pin"></i> <?= htmlspecialchars($t['zona']) ?></div><?php endif; ?>
                        </div>
                        <div class="ct-actions">
                            <?php if ($done): ?>
                                <a class="btn-act ghost" href="aleta.php?id=<?= urlencode($t['cliente_prospecto_id'] ?? '') ?>"><i class="fas fa-eye"></i> Ver aleta</a>
                            <?php else: ?>
                                <a class="btn-act primary" href="nueva_encuesta.php?tarea_id=<?= urlencode($t['id']) ?>"><i class="fas fa-play"></i> Iniciar encuesta</a>
                                <a class="btn-act ghost"   href="aleta.php?id=<?= urlencode($t['cliente_prospecto_id'] ?? '') ?>"><i class="fas fa-folder-open"></i></a>
                            <?php endif; ?>
                        </div>
                    </div>
                <?php endforeach; ?>
