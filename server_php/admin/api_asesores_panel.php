...
try {
    ...
    if ($is_admin) {
        // Gerente: vista global sin filtro de supervisor
        $sql = "
            SELECT
                a.id         AS asesor_id,
                u.nombre     AS asesor_nombre,
                COALESCE(ap.estado, 'desconectado') AS estado,
                ua.latitud, ua.longitud, ua.timestamp AS ultima_vez
            FROM asesor a
            ...
            LEFT JOIN (
                ...
                INNER JOIN (
                    SELECT asesor_id, MAX(timestamp) AS max_ts FROM ubicacion_asesor GROUP BY asesor_id
        ...
        if (!$result) throw new Exception('Error: ' . $conn->error);
    } else {
        // Supervisor: solo sus asesores
        $sql = "
            SELECT
                a.id            AS asesor_id,
                u.nombre        AS asesor_nombre,
