...
## SUPER_IA (XAMPP + PHP backend)

This workspace includes a local PHP backend under `SUPER_IA/server_php` intended to be served by XAMPP/Apache.

### Run on a physical Android phone (recommended: USB tunnel)

If your phone cannot reach your PC over Wi‑Fi/LAN (common with client isolation / firewall), use `adb reverse` so the phone can access your PC's Apache over USB:

1) Start Apache in XAMPP (project path: `C:\xampp\htdocs\SUPER_IA`).
2) Run:
...
This script configures `adb reverse tcp:8080 tcp:80` and runs Flutter with:
`API_BASE_URL=http://127.0.0.1:8080/SUPER_IA/server_php`
